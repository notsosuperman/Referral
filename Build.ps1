...
New-Item -Path "$BUILD_DIR\Tools" -ItemType Directory -Force | Out-Null
New-Item -Path "$BUILD_DIR\Config" -ItemType Directory -Force | Out-Null
Write-Success "Created $BUILD_DIR directory structure"
...
# Compile Tools
Write-Step "Compiling Tools..."

& $AHK_COMPILER /in "Tools\CoordHelper.ahk" /out "$BUILD_DIR\Tools\CoordHelper.exe" | Out-Null
if ($LASTEXITCODE -eq 0) {
    Write-Success "CoordHelper.exe"
} else {
    Write-Error "Failed to compile CoordHelper.ahk"
    exit 1
}
...
# Create empty Config templates (for fresh installs only - Deploy won't overwrite existing)
Write-Step "Creating Config templates..."
@"
; Dental Referral Forms - Control Mappings
; Generated by CoordHelper.ahk
;
; Format: varName=type,x,y[,option1|option2|...]
; Types: checkbox, textfield, multiline, dropdown
; Coordinates are client-relative to "Fill Sheet" window

"@ | Out-File -FilePath "$BUILD_DIR\Config\Mappings.ini" -Encoding UTF8
Write-Success "Mappings.ini template created"

@"
; Dental Referral Forms - Presets
; Created via Ctrl+Shift+S in forms, or manually
;
; Format: [FormName_PresetName]
; key=value pairs matching GUI variable names

"@ | Out-File -FilePath "$BUILD_DIR\Config\Presets.ini" -Encoding UTF8
Write-Success "Presets.ini template created"
